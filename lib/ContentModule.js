...
class ContentModule extends AbstractApiModule {
  ...
  async clone (userId, _id, _parentId, customData = {}, options = {}) {
    ...
    // Fetch all inserted documents in a single query
    const createdDocs = await this.mongodb.find(this.collectionName, { _id: { $in: allNewIds } })
    const docMap = new Map(createdDocs.map(d => [d._id.toString(), d]))
    const createdItems = allNewIds.map(id => docMap.get(id.toString()))

    // Fire postInsertHook on each result
    await Promise.all(createdItems.map(doc => this.postInsertHook.invoke(doc)))

    if (options.invokePostHook !== false) {
      for (let i = 0; i < allItems.length; i++) {
        await this.postCloneHook.invoke(allItems[i], createdItems[i])
      }
    }
    if (originalDoc._courseId?.toString() !== createdItems[0]._courseId?.toString()) {
      await this.updateEnabledPlugins(createdItems[0])
    }

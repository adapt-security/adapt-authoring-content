...
class ContentModule extends AbstractApiModule {
  ...
  async getSchemaName (data) {
    ...
    if (_id && (!_type || !_component)) { // no explicit type, so look for record in the DB
      const [item] = await this.find({ _id }, { validate: false }, { projection: { _type: 1, _component: 1 } })
      if (item) {
        _type = item._type
  ...
  async getSchema (schemaName, data) {
    ...
    const _courseId = data._courseId ??
      (data._id ? (await this.find({ _id: data._id }, { validate: false }, { projection: { _courseId: 1 } }))[0]?._courseId : undefined)
    let enabledPluginSchemas = []
    if (_courseId) {
      try {
        const [config] = await this.find({ _type: 'config', _courseId }, { validate: false }, { projection: { _enabledPlugins: 1 } })
  ...
  async insertRecursive (req) {
    ...
    try {
      ...
      if (rootId === undefined) { // new course
        ...
        if (!parent) {
          throw this.app.errors.NOT_FOUND.setData({ type: 'content', id: rootId })
        }
  ...
  async clone (userId, _id, _parentId, customData = {}, options = {}) {
    ...
    if (!parent && _parentId) {
      [parent] = await this.find({ _id: _parentId }, {}, { projection: { _id: 1, _type: 1, _courseId: 1 } })
    }
    ...
    if (originalDoc._type === 'course') {
      if (tree.config) {
        await this.clone(userId, tree.config._id, undefined, { _courseId: newData._id.toString() }, { tree })
    ...
    for (let i = 0; i < children.length; i++) {
      await this.clone(userId, children[i]._id, newData._id, {}, { tree, parent: newData })
    }
    if (isTopLevel) {
  ...
  async updateSortOrder (item, updateData, parentOptions, parentMongoOptions) {
    ...
    const siblings = await this.find({ _parentId: item._parentId, _id: { $ne: item._id } }, {}, { sort: { _sortOrder: 1 }, projection: { _id: 1, _sortOrder: 1 } })
    if (updateData) {
  ...
  async updateEnabledPlugins ({ _courseId }, options = {}, parentOptions, parentMongoOptions) {
    const { contentplugin, jsonschema } = this
    const contentItems = options.contentItems ?? await this.find({ _courseId }, {}, { projection: { _id: 1, _type: 1, _component: 1, _enabledPlugins: 1, _menu: 1, _theme: 1 } })

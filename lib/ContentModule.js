...
import { ContentTree, buildAssetUsagePipeline, computeSortOrderOps, contentTypeToSchemaName, excludeIdsFromQuery, extractAssetIds, extractSummary, fieldsToProjection, formatFriendlyId, parseMaxSeq, partitionCourseAssets, treeEtag } from './utils.js'
/**
...
class ContentModule extends AbstractApiModule {
  ...
  /**
   * Lists the assets referenced by a course, partitioned into those used only by that course
   * (`deletable` — safe to remove when the course is deleted) and those shared with at least one
   * other course (`shared` — kept). Each row carries display metadata; shared rows also list the
   * other courses keeping the asset alive. Powers the delete-course confirmation preview.
   * @param {String} _courseId The course _id
   * @return {Promise<{ deletable: Array<Object>, shared: Array<Object> }>}
   */
  async getCourseAssets (_courseId) {
    const courseId = parseObjectId(_courseId)
    const items = await super.find({ _courseId: courseId }, {}, { projection: { _assetIds: 1 } })
    const assetIds = [...new Set(items.flatMap(i => (i._assetIds ?? []).map(id => id.toString())))]
    if (!assetIds.length) return { deletable: [], shared: [] }

    const usage = await this.mongodb.getCollection(this.collectionName)
      .aggregate(buildAssetUsagePipeline(assetIds.map(id => parseObjectId(id)))).toArray()
    const courseCounts = Object.fromEntries(usage.map(r => [r._id.toString(), r.courseCount]))
    const { deletable, shared } = partitionCourseAssets(assetIds, courseCounts)

    const assetDocs = await this.assets.find(
      { _id: { $in: assetIds.map(id => parseObjectId(id)) } },
      { validate: false },
      { projection: { title: 1, path: 1, size: 1, type: 1 } }
    )
    const byId = Object.fromEntries(assetDocs.map(a => [a._id.toString(), a]))
    const toRow = id => {
      const a = byId[id] ?? {}
      return { _id: id, title: a.title || a.path || id, size: a.size, type: a.type }
    }
    const otherCourses = await this.coursesReferencingAssets(shared, courseId)
    return {
      deletable: deletable.map(toRow),
      shared: shared.map(id => ({ ...toRow(id), courses: otherCourses[id] ?? [] }))
    }
  }

  /**
   * Maps each of the given assets to the other courses (excluding `excludeCourseId`) referencing it,
   * as `{ _id, title }` rows. Used to explain why a shared asset is kept during course deletion.
   * @param {Array<String>} assetIds Asset ids to resolve
   * @param {ObjectId} excludeCourseId The course being deleted, omitted from the results
   * @return {Promise<Object<String, Array<Object>>>} Asset id to array of course rows
   */
  async coursesReferencingAssets (assetIds, excludeCourseId) {
    if (!assetIds.length) return {}
    const assetSet = new Set(assetIds)
    const rows = await super.find(
      { _assetIds: { $in: assetIds.map(id => parseObjectId(id)) }, _courseId: { $ne: excludeCourseId } },
      {},
      { projection: { _assetIds: 1, _courseId: 1 } }
    )
    const assetToCourseIds = {}
    for (const row of rows) {
      const courseId = row._courseId?.toString()
      if (!courseId) continue
      for (const assetId of (row._assetIds ?? []).map(id => id.toString())) {
        if (assetSet.has(assetId)) (assetToCourseIds[assetId] ??= new Set()).add(courseId)
      }
    }
    const allCourseIds = [...new Set(Object.values(assetToCourseIds).flatMap(s => [...s]))]
    const courses = await super.find(
      { _type: 'course', _id: { $in: allCourseIds.map(id => parseObjectId(id)) } },
      {},
      { projection: { title: 1, displayTitle: 1 } }
    )
    const titleById = Object.fromEntries(courses.map(c => [c._id.toString(), c.displayTitle || c.title]))
    return Object.fromEntries(
      Object.entries(assetToCourseIds).map(([assetId, ids]) =>
        [assetId, [...ids].map(id => ({ _id: id, title: titleById[id] }))])
    )
  }

  /**
   * GET handler backing the delete-course asset preview. Returns {@link ContentModule#getCourseAssets}
   * for the `:_courseId` route param after an access check on the target course.
   * @param {external:ExpressRequest} req
   * @param {external:ExpressResponse} res
   * @param {function} next
   * @return {Promise}
   */
  async handleCourseAssets (req, res, next) {
    try {
      const _courseId = req.apiData.query._courseId
      const course = await this.findOne({ _id: _courseId, _type: 'course' })
      if (!req.auth.isSuper) await this.checkAccess(req, course)
      res.json(await this.getCourseAssets(_courseId))
    } catch (e) {
      next(e)
    }
  }
  ...
  /**
   * DELETE handler for a course. Deletes the course (and its content tree) and, when the
   * `deleteAssets=true` query flag is set, also deletes the assets used only by that course. The
   * exclusive-asset set is captured before the content is removed, then each asset is deleted after —
   * so the assets module's in-use guard sees no remaining references. Assets that turned out to be
   * shared (or already gone) are skipped rather than failing the whole operation.
   * @param {external:ExpressRequest} req
   * @param {external:ExpressResponse} res
   * @param {function} next
   * @return {Promise}
   */
  async handleCourseDelete (req, res, next) {
    try {
      const _courseId = req.apiData.query._courseId
      const deleteAssets = req.query.deleteAssets === 'true' || req.query.deleteAssets === true
      const course = await this.findOne({ _id: _courseId, _type: 'course' })
      if (!req.auth.isSuper) await this.checkAccess(req, course)
      const exclusive = deleteAssets ? (await this.getCourseAssets(_courseId)).deletable.map(a => a._id) : []
      const [deleted] = await this.delete({ _id: course._id })
      const assets = deleteAssets ? await this.deleteExclusiveAssets(exclusive) : { deleted: [], skipped: [] }
      this.log('debug', 'API_DELETE_COURSE', course._id.toString(), 'by', req.auth.user._id.toString(), `assets:${assets.deleted.length}`)
      res.json({ course: deleted, deletedAssets: assets.deleted, skippedAssets: assets.skipped })
    } catch (e) {
      next(e)
    }
  }
  ...
  /**
   * Deletes the given assets one at a time through the assets module (so its delete hooks run,
   * including the in-use guard). An asset that fails — e.g. it became referenced by another course,
   * or was already removed — is collected in `skipped` rather than aborting the run.
   * @param {Array<String>} assetIds Asset ids to delete
   * @return {Promise<{ deleted: Array<String>, skipped: Array<String> }>}
   */
  async deleteExclusiveAssets (assetIds) {
    const deleted = []
    const skipped = []
    for (const _id of assetIds) {
      try {
        await this.assets.delete({ _id })
        deleted.push(_id)
      } catch (e) {
        this.log('warn', `skipped deleting asset ${_id} during course delete: ${e.code || e.message}`)
        skipped.push(_id)
      }
    }
  ...
  async insert (data, options = {}, mongoOptions = {}) {
    ...
    if (doc._type === 'course') { // add the _courseId to a new course to make querying easier
      try {
        return await this.update({ _id: doc._id }, { _courseId: doc._id.toString() })
      } catch (e) { // self-set failed: remove the orphaned row so the insert leaves no partial course behind
        await this.mongodb.deleteMany(this.collectionName, { _id: doc._id }, mongoOptions).catch(() => {})
        throw e
      }
    }
    await Promise.all([

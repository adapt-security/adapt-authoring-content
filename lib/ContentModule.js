...
class ContentModule extends AbstractApiModule {
  ...
  async getSchemaName (data) {
    ...
    if (_id && (!_type || !_component)) { // no explicit type, so look for record in the DB
      const item = await this.findOne({ _id }, { validate: false, throwOnMissing: false }, { projection: { _type: 1, _component: 1 } })
      if (item) {
        _type = item._type
  ...
  async delete (query, options = {}, mongoOptions) {
    ...
    // super.delete for targetDoc (triggers deleteHook middleware for multilang peer cascade)

...
class ContentModule extends AbstractApiModule {
  ...
  async delete (query, options = {}, mongoOptions) {
    ...
    await Promise.all([
      options.updateEnabledPlugins !== false && this.updateEnabledPlugins(targetDoc, { contentItems: remainingItems }, options, mongoOptions),
  ...
  async updateEnabledPlugins ({ _courseId, _type }, options = {}, parentOptions, parentMongoOptions) {
    // only components can change the enabled plugins list, skip for other types unless forced

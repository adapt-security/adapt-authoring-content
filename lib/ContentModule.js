...
class ContentModule extends AbstractApiModule {
  ...
  async getSchemaName (data) {
    ...
    if (_id && (!_type || !_component)) { // no explicit type, so look for record in the DB
      const item = await this.findOne({ _id }, { validate: false, strict: false })
      if (item) {
        _type = item._type
  ...
  async getSchema (schemaName, data) {
    ...
    const _courseId = data._courseId ??
      (data._id ? (await this.findOne({ _id: data._id }, { validate: false, strict: false }))?._courseId : undefined)
    let enabledPluginSchemas = []
    if (_courseId) {
      try {
        const config = await this.findOne({ _type: 'config', _courseId }, { validate: false, strict: false })
  ...
  async clone (userId, _id, _parentId, customData = {}, options = {}) {
    const originalDoc = await this.findOne({ _id })
    if (options.invokePreHook !== false) await this.preCloneHook.invoke(originalDoc)

    const parent = _parentId ? await this.findOne({ _id: _parentId }, { strict: false }) : null
    ...
    if (originalDoc._type === 'course') {
      const config = await this.findOne({ _type: 'config', _courseId: originalDoc._courseId }, { strict: false })
      if (config) {

 ...
 * @param {Number} idInterval Multiplier for non-course IDs
 * @param {String} [_language] Language code (only used for courses)
 * @return {String}
 */
function formatFriendlyId (_type, count, idInterval, _language) {
...
class ContentModule extends AbstractApiModule {
  ...
  async findMaxSeq (_type, _courseId) {
    ...
    return _type === 'course' ? maxNum : Math.floor(maxNum / this.idInterval)
  }
  ...
  async updateEnabledPlugins ({ _courseId, _type }, options = {}, parentOptions, parentMongoOptions) {
    // The caller (update override) gates on whether a plugin-related field changed (_component,
    // _menu, _theme, _enabledPlugins). This guard is still needed because insert() and delete()
    // call unconditionally — skip types that can never affect the plugin list (e.g. page, article).

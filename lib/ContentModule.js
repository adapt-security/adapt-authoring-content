...
import computeSortOrderOps from './utils/computeSortOrderOps.js'
import contentTypeToSchemaName from './utils/contentTypeToSchemaName.js'
import formatFriendlyId from './utils/formatFriendlyId.js'
import parseMaxSeq from './utils/parseMaxSeq.js'
/**
...
class ContentModule extends AbstractApiModule {
  ...
  /**
   * Generates a unique friendly ID using an atomic counter
   * @param {Object} item Must include _type; optionally _courseId and _language
   * @return {Promise<String>}
   */
  async generateFriendlyId (item) {
    const { _type, _courseId, _language } = item
    ...
    if (!exists) {
      ...
      // $setOnInsert ensures only the first concurrent caller seeds; others are no-ops
      await counters.updateOne(query, { $setOnInsert: { seq: maxSeq } }, { upsert: true })
    }
    // Atomically increment — guaranteed unique across concurrent callers
    const counter = await counters.findOneAndUpdate(
      query,
      { $inc: { seq: 1 } },
      { returnDocument: 'after' }
    )
    return formatFriendlyId(_type, counter.seq, this.idInterval, _language)
  }
  ...
  async generateFriendlyIds (_type, _courseId, count, _language) {
    ...
    return Array.from({ length: count }, (_, i) => {
      return formatFriendlyId(_type, startSeq + i, this.idInterval, _language)
    })
  }
  ...
  async findMaxSeq (_type, _courseId) {
    ...
    return parseMaxSeq(docs, _type, this.idInterval)
  }
  ...
  async insert (data, options = {}, mongoOptions = {}) {
    if (!data._friendlyId) {
      data._friendlyId = await this.generateFriendlyId(data)
    }
    let doc
    try {
  ...
  async clone (userId, _id, _parentId, customData = {}, options = {}) {
    ...
    if (!tree) {
      tree = new ContentTree(await this.find({ _courseId: originalDoc._courseId }))
    }
    // Pre-allocate friendly IDs in bulk for the entire subtree (top-level only)
    if (isTopLevel && !options._friendlyIds) {
      const descendants = tree.getDescendants(_id)
      const allItems = [originalDoc, ...descendants]
      if (tree.config && originalDoc._type === 'course') {
        allItems.push(tree.config)
      }
      const targetCourseId = parent?._type === 'course' ? parent?._id?.toString() : parent?._courseId?.toString()
      const typeCounts = new Map()
      for (const item of allItems) {
        if (item._type === 'course' || item._type === 'config') continue
        const key = item._type
        typeCounts.set(key, (typeCounts.get(key) ?? 0) + 1)
      }
      const friendlyIds = new Map()
      await Promise.all([...typeCounts].map(async ([_type, count]) => {
        const ids = await this.generateFriendlyIds(_type, targetCourseId ?? originalDoc._courseId, count)
        friendlyIds.set(_type, ids)
      }))
      options = { ...options, _friendlyIds: friendlyIds }
    }
    const friendlyIds = options._friendlyIds
    let friendlyId
    if (originalDoc._type === 'course') {
      friendlyId = originalDoc._friendlyId
    } else if (originalDoc._type === 'config') {
      friendlyId = formatFriendlyId('config')
    } else {
      friendlyId = friendlyIds?.get(originalDoc._type)?.shift()
    }
    const schemaName = contentTypeToSchemaName(originalDoc._type)
    ...
    try {
      ...
      if (originalDoc._type === 'course') {
        if (tree.config) {
          await this.clone(userId, tree.config._id, undefined, { _courseId: newData._id.toString() }, { tree, _createdItems: createdItems, _friendlyIds: friendlyIds })
      ...
      await Promise.all(children.map(c => {
        return this.clone(userId, c._id, newData._id, {}, { tree, parent: newData, _createdItems: createdItems, _friendlyIds: friendlyIds })
      }))
      if (isTopLevel && originalDoc._courseId?.toString() !== newData._courseId?.toString()) {
      ...
      if (isTopLevel && createdItems.length > 0) {
        await super.deleteMany({ _id: { $in: createdItems.map(c => c._id) } }, { invokePostHook: false })
      }
      throw e
    }
  }

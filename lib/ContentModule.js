...
class ContentModule extends AbstractApiModule {
  ...
  async clone (userId, _id, _parentId, customData = {}) {
    ...
    const newData = await this.insert(AbstractApiUtils.stringifyValues(payload), { schemaName })
    ...
    if (originalDoc._type === 'course') {
      ...
      // schemas will have therefore stripped the plugin configuration data
      // here we restore the configuraton data
      await this.update({_id:newData._id}, AbstractApiUtils.stringifyValues(payload))
    }
